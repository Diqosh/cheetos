    ...
    <div className="py-20 relative" ref={componentRef}>
      <div className="container flex flex-col mx-auto px-4 relative z-10">
        ...
        >
          ...
          <div className={`absolute ${tigerChange ? "left-1/2 top-[26px]" : "left-[270px] sm:left-[310px] md:left-[370px] lg:left-[650px] -top-[60px]"} ${tigerChange ? "" : "rotate-45"}`}>
            <img
              src={tiger}
        ...
        >
          <motion.p
            className={`py-auto lg:text-2xl lg:text-3xl w-full ${tigerChange ? "mx-auto w-[331px]" : ""}`}
            initial={{ opacity: 0, y: 50 }}
            animate={isVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
            transition={{ duration: 0.6, delay: 0.4 }}
          >
            ЧЕСТЕР ПОДГОТОВИЛ ДЛЯ ТЕБЯ ТЕСТ СО СЛУКОМ ДЛЯ ВСЕХ,
            ВЛЮБЛЕННЫХ ОТКРЫЛИ ЧЕСТОС ИХ 9 ВОПРОСОВ, ЧТОБЫ УЗНАТЬ,
            КАКАЯ РОЛЬ ПОДХОДИТ ТЕБЕ НА 14 ФЕВРАЛЯ И ЧТО КАКОЙ ТЫ
            ВЛЮБЛЕНЫ?
          </motion.p>
          <motion.div
